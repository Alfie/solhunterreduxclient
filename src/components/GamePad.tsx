...
export const GamePad: FC = () => {
    ...
    const onSquareClick = useCallback( async (idx) => {
        ...
        if (idx == 1) {
            ...
            signature = await movePlayer(wallet,1, publicKey);
        }

        if (idx == 3) {
            console.log("***Pressed left")
            signature = await movePlayer(wallet,2, publicKey);
        }

        if (idx == 5) {
            console.log("***Pressed right")
            signature = await movePlayer(wallet,0, publicKey);
        }

        if (idx == 7) {
            console.log("***Pressed down")
            signature = await movePlayer(wallet,3, publicKey);
        }
      }, []);
